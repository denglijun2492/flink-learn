...
public class PunctuatedAssignerDemo {
...
//        DataStream<String> dataStream = environment.readFile(new TextInputFormat(new Path(path)), path, FileProcessingMode.PROCESS_CONTINUOUSLY, 100L);
        ...
        DataStream<String> result = dataStream.assignTimestampsAndWatermarks(new MyPunctuatedAssigner()).map(new MapFunction<String, Gjxx>() {
            @Override
